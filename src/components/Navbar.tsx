    ...
    <>
      <nav className="fixed top-0 left-0 w-full z-50 mix-blend-difference text-white">
        <div className="flex justify-between items-center px-6 md:px-8 py-4 md:py-6 max-w-[1920px] mx-auto">
          {/* Logo */}
          <div className="font-sans font-bold text-lg md:text-xl tracking-tighter cursor-pointer z-50" onClick={() => setIsOpen(false)}>
            SINERGIE SRL
          </div>
          ...
          <div className="hidden md:flex gap-8 lg:gap-12 font-sans text-xs md:text-sm font-semibold tracking-wider">
            {['HOME', 'SERVIZI', 'LAVORI', 'CHI SIAMO', 'CONTATTI'].map((item) => (
          ...
          <div className="flex items-center gap-4 md:gap-6 z-50">
            <a href="#contatti" className="hidden sm:block text-xs md:text-sm font-bold tracking-wider hover:opacity-70 transition-opacity">
              CONTATTACI
            </a>
        ...
        {isOpen && (
          ...
          >
            <div className="flex flex-col space-y-6 md:space-y-8 text-center">
              {['HOME', 'SERVIZI', 'LAVORI', 'CHI SIAMO', 'CONTATTI'].map((item, index) => (
                <motion.div
                  key={item}
                ...
                >
                  <a 
                    ...
                    className="text-4xl md:text-6xl lg:text-8xl font-sans font-black tracking-tighter hover:text-gray-400 transition-colors"
                  >
                    {item}
                  </a>
                </motion.div>
